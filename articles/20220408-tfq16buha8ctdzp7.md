...
**Vite**がめっちゃ速いらしいということで、今更ではありますが、触って遊んでいます。
その中で**Pug**を使おうとすると一筋縄ではいかなかったので、Pugを使えるようにするまでの手順をまとました。

「vite pug」等で検索するとVueやReact、HTMLを噛ませる方法はヒットするのですが、Pugを単体で使う方法はなかなかヒットしません。なのでVite上でPugを単体で使えるようにしようというのが本記事の趣旨です。
