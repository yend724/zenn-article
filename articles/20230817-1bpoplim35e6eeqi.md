...
上記では書籍一覧画面と書籍詳細画面を一つの HTML ファイルに記述しています。初期状態では書籍詳細画面に `hidden` 属性を付与し、`display:none` で要素の表示を消しています。今回はこの二つの画面の表示を切り替えることで、擬似的にページ遷移を行なっていると考えてください。

リンクをクリックすると、アニメーション付きの画面遷移が行われています。JavaScript でどのように実装されているのか確認してみましょう。
...
`startViewTransition(updateCallback)` の `updateCallback` には、DOM の更新処理を記述します。今回は書籍一覧画面と書籍詳細画面の表示を切り替えているだけです。驚くべきことに、たったこれだけの記述でクロスフェード遷移を伴った DOM の更新が行われます。
...
Navigation API は [`window.navigation`](https://developer.mozilla.org/en-US/docs/Web/API/Window/navigation) を介してアクセスすることができます。
...
実際にページの挙動を確認してみると（アニメーションを実装していないこともあり）一見通常の `a要素` によるページ遷移と違いがないように思えるかもしれません。
