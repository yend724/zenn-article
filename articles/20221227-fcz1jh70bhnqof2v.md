...
カスケードレイヤー`@layer`がモダンブラウザでサポートされたとはいえ、互換性の問題から、実際に現場で見かけるのは、まだまだ未来の話と思っていました。ふと、思い立って`Can I use... Support tables for HTML5, CSS3, etc`を確認してみたところ、現時点（2022.12.27）でも、すでにGlobalで９割近いサポート率となっています。これは来年あたりから見かける機会が増えそうだと思ったので、そろそろカスケードレイヤーと向き合いたいと思います。
...
上記のようなHTMLとCSSの記述があるとします。詳細度の比較において、IDセレクターはクラスセレクターよりも優先されるため、上記のコードでは`color: red`が適用されます。

次にカスケードレイヤーを導入した例を見てみます。
...
`@layer`を使用して、`base`と`overwrite`の2つのレイヤーを宣言しています。カスケードレイヤーのアルゴリズムは、詳細度よりも優先されるため、上記の場合は`overwrite`レイヤーの`color: blue`が適用されます。

## 基本構文

カスケードレイヤーには、3種類の宣言方法があります。
...
まずはブロック構文と`@layer`による宣言について、確認してみます。
...
一文構文ではスタイルの伴わないレイヤーを宣言することができます。こちらはブロック構文とは異なり、レイヤーの優先順位のみに関与します（[レイヤーの順序](#レイヤーの順序)を参照）。
...
`@import`でインポートしたCSSに対して、レイヤーを宣言することもできます。この場合、`@layar`ではなく、`layer関数`か、もしくは`layerキーワード`を使用します。
...
一方で、`layerキーワード`を用いて宣言した場合、それは匿名レイヤーとして宣言されることになります。
...
カスケードレイヤーは、各レイヤーが最初に宣言された順序でソートされます。この時、**後にくるレイヤー**の優先順位が高くなります。
...
ここで、暗黙的な外縁レイヤーという言葉が急に登場しました。暗黙的な外縁レイヤーとは、`@layer`、`layer関数`、`layerキーワード`を使用して明示的に宣言されたレイヤーの、外にあるスタイルのことです。上記の例でいう`.hoge { color: red }`の部分のことであり、レイヤー化されていない（unlayered）スタイルともいえます。
...
上記のコードにおいてレイヤーの順序は以下のようにソートされています。
...
`(暗黙的な外縁レイヤー)`の優先順位は、変わらず最も高いことに注意してください。`reset`、`base`、`overwrite`の順序は一文構文で簡単に並び替えることができます。
...
レイヤーを入れ子にした際、レイヤーのネストが深くなれば深くなるほど、優先順位は低くなります。上記の例において、レイヤーの順序は以下のようにソートされます（後にくるレイヤーの方が優先順位が高いことに注意してください）。
...
この場合、`(暗黙的な外縁レイヤー)`のスタイルが存在しないので、`framework`レイヤーである`.hoge { color: red }`が優先されることになります。
...
次のように同じ名前のレイヤーを複数宣言した場合、後に書いたレイヤーのスタイルは既存のレイヤーに追加されます。次の例では`color: red`が適用されます。
...
次の例では、`base`が最初に宣言されているので、`overwrite`よりも優先されます。各レイヤーが最初に宣言された順序でソートされることを、忘れないようにしましょう。
...
匿名レイヤーは、宣言ごとに独立した個別のレイヤーを作成します。次のコードでは`base`レイヤーを2つのブロック構文に分けて記述しています。この場合、2つ目のブロック構文のスタイルは、既存のレイヤーに追加されます。
...
次のようなコードにおいて、名前があるレイヤーと匿名レイヤーで、挙動が変わることがあるので注意しましょう。
...
匿名レイヤーは、そのレイヤーに対して外部からスタイルを上書きする、もしくは追加する手段を提供しません。あくまで一例ですが、これは以下のようなケースで効果を発揮することがあります。
...
レイヤー化されたスタイルとインラインスタイルでは、インラインスタイルが優先されます。次の例では、`color:red`が適用されます。
...
レイヤー化されたスタイルと`!important`を付与されたスタイルでは、`!important`を付与されたスタイルが優先されます。
...
実際に例をみて確認しましょう。ややこしいことに、次のコードでは`color: green !important`が適用されます。
...
ここまでは本記事において何度かやってきたことであり、特に問題ないと思います。通常では暗黙的な外縁レイヤーが優先されるのですが、`!important`同士の比較では、レイヤーの優先順位が逆転します。つまりレイヤーの順序が以下のようにソートされます。

1. (暗黙的な外縁レイヤー) - `!important`
2. `overwrite`レイヤー - `!important`
3. `base`レイヤー - `!important`

したがって、`base`レイヤーの`!important`の優先順位が最も高くなり、上記の例では`color: green !important`が適用されることとなります。

# おわりに

いかがでしたでしょうか。カスケードレイヤーと向き合う話でした。案外ややこしい部分もあったので、導入する際には事前に知識を整理しておく必要があります。カスケードレイヤーは、便利である一方で、その影響範囲も大きいので、やはりCSS設計をしっかり考えておくことが重要ですね。
