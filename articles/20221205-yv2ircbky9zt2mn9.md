...
上記は`CSS Nesting`と`Custom Media Queries`を導入した記述になります。上記のスタイルを適用した場合、次のよう画像のように表示されます。
...
画像をみてもわかるように、`CSS Nesting`と`Custom Media Queries`が反映されているがわかります。
...
Lightning CSSはParcel以外でも簡単に使うことができました。
