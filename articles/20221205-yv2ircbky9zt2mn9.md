...
:::message
本記事では自作プラグインを作成しますが、[公式ドキュメント](https://lightningcss.dev/docs.html#with-vite)に既存プラグインを使用する方法が載っています。既存プラグインを使用する場合、本記事執筆時点（2022.12.05）ではオプションの設定がまだ実装されていないようでした。
:::
