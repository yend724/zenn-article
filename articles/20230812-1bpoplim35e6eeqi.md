...
あとは `a` をクリックした際に `swap()` を実行するだけです。書籍一覧画面と書籍詳細画面との間でアニメーション付きの画面遷移が行われます。

ここでデモサイトの挙動を観察してみると、画面全体のクロスフェード遷移（`main` 要素全体の遷移）と、異なるアニメーションを行っている要素が存在することに気付くと思います。

具体的に全体と異なるアニメーションの遷移をしている要素は次の二つになります。
...
上記の要素は画面遷移間で、要素自体が（クロスフェードでなく）移動しているように見えます。これらの要素に対して当たっているスタイルを確認してみましょう。
...
ここで View Transitions API を使うと、どのような流れで画面遷移が行われるかをざっくり紹介します。
...
View Transitions API は画面遷移の前後で状態のビジュアル的なスナップショットを生成します。上記の流れから分かるように、実際に遷移中のアニメーションを実現するのは遷移擬似要素と呼ばれる擬似要素です。実際にディペロッパーツールで確認しみると、`html` 要素の直下に、次のような構造の擬似要素ツリーが生成されているのがわかります。
...
本記事では遷移擬似要素の詳細な説明は行わず、ざっくり紹介します。

- [::view-transition](https://drafts.csswg.org/css-view-transitions/#selectordef-view-transition) は
- [::view-transition-group()](https://drafts.csswg.org/css-view-transitions/#selectordef-view-transition-group) は
- [::view-transition-old()](https://drafts.csswg.org/css-view-transitions/#selectordef-view-transition-old) は
- [::view-transition-new()](https://drafts.csswg.org/css-view-transitions/#selectordef-view-transition-new) は
...
次に Navigation API について確認してみようと思います。
...
本記事で紹介する Navigation API の実装におけるデモページ&コードになります。
...
`loadNextPage()` 内では NavigateEvent の [`destination`](https://developer.mozilla.org/en-US/docs/Web/API/NavigateEvent/destination) で得たナビゲーション先の URL を、[`fetch()`](https://developer.mozilla.org/ja/docs/Web/API/fetch) に渡し、を取得した HTML 文字列を [`DOMParser`](https://developer.mozilla.org/ja/docs/Web/API/DOMParser) で DOM にパースしています。その後 `swap()` 関数で、現在のページの画面

上記のコードでは、置き換えているのはあくまで `*[data-transition-wrapper]` のセレクターで取得できる範囲の話なので、その後 `document.title` も書き換えています。

`shouldNotIntercept()`に関して、詳しいことは [Chrome Developers による記事](https://developer.chrome.com/docs/web-platform/navigation-api/#deciding-how-to-handle-a-navigation)を参照してください。どのようなナビゲーションイベントをインターセプトすべきでないのかについて確認できます。
...
Navigation API のデモについて一通り確認したわけですが、実際にページの挙動を確認してみると（アニメーションを実装していないこともあり）一見通常の `a` 要素によるページ遷移と違いがないように思えるかもしれません。

Chrome DevTools の Network タブを確認してみると、ページ遷移するたびに非同期で次のコンテンツを `fetch` してることがわかります。

従来の方法である`History API` で似たようなことをすると、`a` 要素のクリックイベントをハンドリングしたり、[`history.pushState()`](https://developer.mozilla.org/ja/docs/Web/API/History/pushState) で履歴の管理を行う必要があったのですが、`Navigation API`の登場で、これらがより簡易で柔軟に実現できるようになりました。

## View Transitions API と Navigation API を組み合わせる

それでは、ここまでに扱ってきた View Transitions API と Navigation API を組み合わせてページ遷移アニメーションを実装してましょう。
...
![View Transitions API と Navigation API デモ画面](/images/20230812/view-transitions-api-demo.webp)

### View Transitions API と Navigation API を用いたページ遷移アニメーションの実装

ここではこれまでに登場してきた View Transitions API と Navigation API を組み合わせるだけなので、これといって目新しい話があるわけではありません。とりわけコードに関してはほとんど変更がないので、変更がある箇所のみ記載します。
...
JavaScript に関しては、Navigation API のデモコードからほとんど変更がありませんが、`swap()` の中身を `document.startViewTransition()` で処理しています。

View Transitions API と Navigation API を用いたページ遷移アニメーションのデモコードについては、以上となります。実際にページ遷移の挙動を確認してみると、要素がアニメーションしながら非同期で遷移していることがわかると思います。

## Cross Document View Transitions

さて、ここでもう一歩先に進んだ [Cross Document View Transitions](https://github.com/WICG/view-transitions/blob/main/cross-doc-explainer.md) にも、軽く触れてみようと思います。

:::message
Cross Document View Transitions は、本記事執筆時点（2023.8.16）において、Google Chrome の [chrome://flags/#view-transition-on-navigation](chrome://flags/#view-transition-on-navigation) を Enabled にすることで使用できます。
:::

ここまでに触れてきた View Transitions API は主に同一ドキュメント向けの内容でした。つまり MPA（Multi-Page Application）では使用できず、ページ間で画面遷移を行いたい場合は `fetch()` で非同期的に次のページの内容を取得する必要があります。

一方で異なるページ（ドキュメント）間での View Transitions API の対応も考えられているようで、現時点では以下のように `meta` タグを宣言することで使えるようになるようです。

```html
<meta name="view-transition" content="same-origin" />
```

この際、単純に最小限のアニメーション遷移をするだけであれば JavaScript の記述も不要になります（なんと `fetch()` する必要もなくなります）。
