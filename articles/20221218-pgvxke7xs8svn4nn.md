...
最終的に適用されるのは`p#hoge.fuga { color: red; }`でした。いかかでしょうか。手順に当てはめて比較していくだけなので、そこまで難しい作業ではないかもしれません。非常に複雑な指定でもない限り、慣れてくると案外すぐわかるようになります。

これまでの説明でわかるようにA、B、Cの値には超えられない壁があります。仮にIDセレクター1つに対し、クラスセレクターが100個あったとしても、前者が優先されます。*(1, 0, 0)* と *(0, 100, 0)* の比較では、前者が勝つということです。

# 詳細度をコントロールする

CSSの記述において、詳細度をコントロールすることは非常に重要です。ここでは詳細度をコントローするためのテクニックやTIPSを紹介しようと思います。
...
セレクターの具体性を高めずに詳細度を高めたければ、複合セレクターの重複を使用できます。
...
まずわかりやすい`:where()`から確認してみましょう。`:where()`はその引数のセレクターの詳細度を0にします。`:where()`を使うことで、セレクターの具体性を高めながら、詳細度を低く保つことができます。
...
通常であればインラインスタイルを記述する機会は少ないかもしれませんが、`JavaScript`から要素を操作したり、CMSなどでバックエンド側から何かしらのスタイルを適用させたいときに有用となります。
...
カスケードレイヤー > 詳細度」という順番で宣言が優先されます。ただし、`!important`とカスケードレイヤーが絡むことで、カスケードレイヤーの優先順位に逆転が生じることがあります。もし興味がありましたら、別途記事を書きましたのでご参照ください。

https://zenn.dev/yend724/articles/20221227-fcz1jh70bhnqof2v
