...
published: true
published_at: 2024-02-02 13:00
---

## はじめに

テキストの読みやすさを大きく左右する要素の1つに、テキストと背景のコントラストがあります。これまで私は、何かしらのツールを用いてコントラスト比を確認することはあれど、その具体的な計算方法については理解していませんでした。そこで、今回は[Web Content Accessibility Guidelines (WCAG) 2.2](https://www.w3.org/TR/WCAG22/)を参考に、コントラスト比の計算方法について学んだのでご紹介します。

## ウェブアクセシビリティにおけるコントラストとは

一般的に「コントラスト」という言葉には「対比」や「対照」という意味があります。とりわけ、ウェブアクセシビリティにおいてコントラストという言葉は、テキストと背景のコントラストを指すことがあります。
...
:::message alert
本記事では触れませんが、WCAG3ではAPCAという手法を用いた新しいコントラストの基準が開発・検討されているようです。本記事はWCAG2.2をもとに記載しており、WCAG3の内容とは大きく異なる可能性がありますので、ご注意ください。
参考：[Why APCA as a New Contrast Method? | APCA](https://git.apcacontrast.com/documentation/WhyAPCA)
:::
...
ここで、$L{\sixptsize 1}$ は明るい色の相対輝度、$L{\sixptsize 2}$ は暗い色の相対輝度を表します。相対輝度については[後述](#%E7%9B%B8%E5%AF%BE%E8%BC%9D%E5%BA%A6%E3%81%AE%E8%A8%88%E7%AE%97%E6%96%B9%E6%B3%95)しますが、上記の式からわかるようにコントラスト比は明るい色と暗い色の比を表します。

またコントラスト比は 1 ～ 21 の範囲に収まります（通常は 1:1 ～ 21:1 のように表記される）。

### 相対輝度の計算方法

[相対輝度](https://www.w3.org/TR/WCAG22/#dfn-relative-luminance)という見慣れない単語が登場しましたが、こちらもWCAG2.2に記載があります。

> the relative brightness of any point in a colorspace, normalized to 0 for darkest black and 1 for lightest white

上記によると、**相対輝度は色空間内における任意の点の相対的な明るさ**のことです。最も暗い黒は0、最も明るい白は1に正規化されます。
...
今回はコントラストの計算方法について学びました。具体的な計算方法を学ぶことで、曖昧だったコントラストへの理解が少し深まった気がします。本記事の内容が、誰かのお役に立てれば幸いです。
