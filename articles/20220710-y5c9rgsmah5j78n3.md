...
本記事で参考にしてきたARIA APGのアコーディオンの項目では、ページ内検索については触れていませんでした。[Accordion Example](https://www.w3.org/WAI/ARIA/apg/example-index/accordion/accordion.html)では、アコーディオンが折りたたまれた状態でページ内検索をしても隠されている単語はヒットしません。
...
以上が`hidden="until-found"`の実装になります。
